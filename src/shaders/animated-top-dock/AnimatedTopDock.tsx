...
export const PORTFOLIO_DOCK_ITEMS: readonly DockItem[] = [
  {
    id: "about",
    label: "About",
    href: "#about",
    icon: (
      <>
        ...
        <path d="M2.5 13.8c0-2.8 2.5-4.8 5.5-4.8s5.5 2 5.5 4.8" />
      </>
    ),
  },
  {
    ...
    icon: (
      <>
        ...
        <path d="M5.5 4V2.6a1 1 0 0 1 1-1h3a1 1 0 0 1 1 1V4M2 7.5h12" />
      </>
    ),
  },
  {
    ...
    icon: (
      <>
        <path d="m5 4.5-3.2 3.5 3.2 3.5M11 4.5l3.2 3.5-3.2 3.5M9.2 2.8 6.8 13.2" />
      </>
    ),
  },
  {
    ...
    icon: (
      <>
        <polygon points="8 1.5 10 5.8 14.5 6.5 11.2 9.7 12 14.2 8 12.1 4 14.2 4.8 9.7 1.5 6.5 6 5.8 8 1.5" />
      </>
    ),
  },
  {
...
export function AnimatedTopDock({ className = "", ...props }: AnimatedTopDockProps) {
  ...
  const [active, setActive] = useState(props.activeId ?? items[0]?.id ?? "");
      ...
      <div className={`animated-top-dock-component atd-modern${className ? ` ${className}` : ""}`}>
        ...
        <header className="atd-modern__bar">
          <a
            ...
            href={props.brandHref ?? "#top"}
            onClick={(event) => {
              if (props.brandHref?.startsWith("#")) {
                event.preventDefault();
                const target = document.querySelector(props.brandHref);
                if (target) target.scrollIntoView({ behavior: "smooth" });
                else window.scrollTo({ top: 0, behavior: "smooth" });
              }
            }}
          >
            <span className="atd-modern__mark" aria-hidden="true">{props.brandMark ?? BRAND_MARK}</span>
            <span className="atd-modern__word">{props.brandName || "Lumina"}</span>
            {props.brandBadge && (
              <span className="text-[9px] font-mono font-semibold px-2 py-0.5 rounded-full bg-white/[0.08] text-[#B8C4CC] border border-white/10 hidden sm:inline-block">
                {props.brandBadge}
              </span>
            )}
          </a>
